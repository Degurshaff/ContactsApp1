...
namespace ContactsAppUI
{
    ...
    public partial class MainForm : Form
    {
        ...
        /// </summary>
        const string BirtdaysStringStart = ""; //мелкие буквы
        ...
        private void Edit()
        {
            ...
            var form = new ContactsForm { Contact = selectedContact };
            var dialogResult = form.ShowDialog();
            if (dialogResult == DialogResult.OK)
            {
        ...
        private void CheckForBirthday()
        {
            var birthdays = from contact in _project._contactlist
                            ...
                            select contact;
            var birthdayData = _project.BirthData(DateTime.Today);
            if (birthdayData.Count != 0)
            {
                foreach (var birthdayDat in birthdayData)
                {
                    ...
                    BirthdayTextBox.Text = BirtdaysStringStart + string.Join(", ", birthdaysSurnames);
